...
void example_1()
{
	...
	{ // -- alloc memory for the vertex buffer
		...
		vkGetPhysicalDeviceMemoryProperties(vk.physicalDevice, &deviceMemProps);
		u32 memoryTypeToAllocateFrom = -1;
		for (u32 memTypeInd = 0; memTypeInd < deviceMemProps.memoryTypeCount; memTypeInd++) {
			auto& memType = deviceMemProps.memoryTypes[memTypeInd];
	...
	{ // -- create cmd buffers
		...
		vkRes = vkAllocateCommandBuffers(vk.device, &info, vk.cmdBuffers);
		assert(vkRes == VK_SUCCESS);
	}
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		const VkSubmitInfo submitInfo = {
			...
			.pCommandBuffers = &vk.cmdBuffers[frameInd],
